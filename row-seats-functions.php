...
$errors= apply_filters('row_seats_custom_field_error',$errors);
		...
		if(count($errors)>0)
		{
			...
			$cartitems = json_decode(base64_decode($_POST['mycartitems']), true);
//print "<br><br><br>";//print base64_decode($_POST['mycartitems']);
			//print_r($cartitems);
			//print "<br><br><br>";
...
$data['myitems']=$myitems;			
		$data['event_name_display']=$eventname1." ".implode(",",$mytseats);
			...
			{
			$data['feeifany']=number_format($_POST['rst_fees'], 2, ".", ""); // fix for paypal decimal issue
...
function addQueryParam( url, key, val ){
  ...
  function addParameterToURL(_url,_key,_value){
      ...
      return _url;
		...
		var newurl=addParameterToURL( window.location.href, 'stoprefresh', 'yes' );
		
		var newurl=addParameterToURL( newurl, 'returnurl', window.location.href );
		//alert('--------' + newurl);
...
function rst_shows_operations($action, $data, $currentcart)
{
    ...
    switch ($action) {
	   ...
	   $cartitems = json_decode($currentcart,true);
			$rst_options = get_option(RSTPLN_OPTIONS);	
			...
			$currentcart = json_decode($currentcart,true);			
			//print_r($currentcart);
            for ($i = 0; $i < count($currentcart); $i++) {
	    ...
	    return $data['bookingid'];
            ...
            VALUES ($showid, '$rst_session_id', '$paypal_vars',now(),'$booking_detail','$status','$username','$useremail','$phone','$papalmode','','$user_id')";						//print $sql;
        ...
        case 'delete':
            $id = $data['id'];
            // print_r($data);
            $wpdb->query("DELETE FROM rst_shows WHERE id='".$id."'");
			//$wpdb->delete( $wpdb->rst_shows, array( 'id' => $id ) );
    ...
    if (isset($_POST['action'])) {
		...
		//$details = json_decode($details,true);
            ...
            case 'deleteall': // {
                ...
                $details = base64_decode($_POST['cartiterms']);
                $currentcart = json_decode($details,true);
		...
		//$currentcart2 = base64_decode($currentcart);                    
		$currentcart2 = unserialize($currentcart);					 
		$currentcart2 = json_decode($currentcart2,true);																
...
$seatsize=0;
    ...
    $rst_bookings = json_decode($rst_bookings,true);
	$divwidth=apply_filters('row_seats_table_divwidth',$divwidth,$showid);
...
$showcode="row_seats_amenities_".$_SESSION['views'];
if($rst_options[$showcode] && is_plugin_active( 'row-seats-amenities/row-seats-amenities.php' ))
{
...
?>		
	...
	document.getElementById('amount').value=gtotal;
	    ...
	    //print_r($bookings);
           // $mycartitems = json_encode($bookings);
...
$showcode="row_seats_amenities_".$_SESSION['views'];
if(isset($rst_options[$showcode]) && is_plugin_active( 'row-seats-amenities/row-seats-amenities.php' ))
{
...
$topheader='<div class="stage-hdng" style="margin-left: 0px; margin-top: 0px; margin-right: 0px; margin-bottom: 10px;width:' . $divwidth . 'px; border:1px solid;border-radius:5px;box-shadow: 5px 5px 2px #888888;" >'.$event_seat_stage.'</div>';
    ...
    $rst_bookings = $bookings;
    $sessiondata = base64_encode(json_encode($rst_bookings));
    if ($sessiondata != "") {				
        ?>
        <script>
...
function rst_shows_set_session($data, $action, $currentcart)
{
    if ($action == 'add') {
	   ...
	   $currentbooking = json_decode($currentcart,true);
        }
        if (count($data) > 0) {
...
unset($booking_details['customfield']);
		...
		//print_r($booking_details);
        $mytickets=array();
		$seatcosts=array();		//print_r($booking_details);			//	exit;
        for ($row = 0; $row < count($booking_details); $row++) {
            $seats = $booking_details[$row]['seatno'];
            $showid = $booking_details[$row]['show_id'];
            $rowname = $booking_details[$row]['row_name'];
			...
			{
            $sql = "INSERT INTO $wpdb->rst_booking_seats_relation (ticket_no,ticket_seat_no,booking_id,show_id,b_seatid,total_paid,txn_id,seat_cost,booking_status,comments)
            VALUES ('$ticketno', '$ticket_seat_no', $booking_id,$showid,$seatid,$totalpaid,'$txn_id',$price,'','')";//print  $sql;//exit;
            $wpdb->query($sql);
			}
        }
