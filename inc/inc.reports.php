...
else
{
	...
	//exit;
}//Amenities - Display amenities booked- START$bookdetails=unserialize ($item['booking_details']);$amenities=$bookdetails['amenties'];//print $amenities;if(isset($amenities)){	        $symbol = get_option('rst_currencysymbol');        $symbols = array(            "0" => "$",            "1" => "&pound;",            "2" => "&euro;",            "3" => "&#3647;",            "4" => "&#8362;",            "5" => "&yen;");        $symbol = $symbols[$symbol];			$vairablenamevalue=unserialize(base64_decode($amenities));	if(is_array($vairablenamevalue))	{		for($am=0;$am<count($vairablenamevalue);$am++)		{			if($vairablenamevalue[$am]['label'] && $vairablenamevalue[$am]['quantity'])			{								echo $vairablenamevalue[$am]['label'].":".$symbol.number_format($vairablenamevalue[$am]['price'], 2, ".", "")."*".$vairablenamevalue[$am]['quantity'].'='.$symbol.number_format($vairablenamevalue[$am]['pricetotal'], 2, ".", "")."<br>";			}		}	}	}//Amenities - Display amenities booked- ENDexit;
}
